...
import { formatFechaNoteCard, toPastelColorBkgnd, validarIDOConvertirElemento } from '../utils.js';
// import { actualizarGrupoActivoUI } from './GroupManager.js';
import { obtenerNotaPorIdDesdeDB, guardarNotaEnDB, eliminarNotaDeDB, actualizarOrdenGruposEnDB } from '../services/db.js';
import { abrirEditorNota } from './NoteEditor.js';
import { initNoteEditor } from './NoteEditor.js';

import Muuri from 'muuri';

import { gridUnpinned, gridPinned, ordenarNotas } from '../main.js';
...
export const renderizarNotaEnDOM = (noteData) => {
    ...
    // Si ya existía, ya está en su sitio, no hace falta añadirla de nuevo.
    if (existingNoteCard) {
        verificarYReubicarNota(existingNoteCard.id, noteData.pinned);
    }
...
export const actualizarInfoGrupoEnNoteCards = (groupID, updates = {}) => {
    // Caso especial: Limpiar notas de grupos que ya no existen (cuando se llama con groupID = null).
    if (groupID === null || !groupID) {
        const allGroupElements = document.querySelectorAll('.notes-group');
        const existingGroupIDs = Array.from(allGroupElements).map(el => el.id);
        // const allNotes = document.querySelectorAll('#all-notes-container .note-card-container');
        const allNotes = document.querySelectorAll('.grid .note-card-container, #pinned-notes-container .note-card-container');

        allNotes.forEach(note => {
            const noteGroupID = note.dataset.groupId;
            if (noteGroupID && noteGroupID !== 'null' && !existingGroupIDs.includes(noteGroupID)) {
                note.setAttribute('data-group-id', 'null');
                const groupColorEl = note.querySelector(".note-group-color");
                const groupNameEl = note.querySelector(".note-group-name");
                ...
                note.style.backgroundColor = toPastelColorBkgnd(groupColorDefault);
                // console.log(`Nota '${note.id}' actualizada. Grupo '${noteGroupID}' ya no existe.`);
            }
        });
        return;
    }
    ...
    const notesInGroup = document.querySelectorAll(`.grid [data-group-id="${groupID}"]`);

    notesInGroup.forEach(note => {
        if (updates.color) {
            const groupColorEl = note.querySelector(".note-group-color");
            if (groupColorEl) groupColorEl.style.backgroundColor = updates.color;
            note.style.backgroundColor = toPastelColorBkgnd(updates.color);
        }
        if (updates.name) {
            const groupNameEl = note.querySelector(".note-group-name");
...
export const actualizarInfoGrupoEnNoteCardspecific = (noteID, groupID) => {

    const noteSpecific = document.getElementById(noteID);
    const noteGroupColor = noteSpecific.querySelector(".note-group-color");
    const noteGroupName = noteSpecific.querySelector(".note-group-name");
    ...
    if (noteGroupColor && noteGroupName) {
        ...
        if (!groupID || groupID === 'null') {
            ...
            noteSpecific.style.backgroundColor = toPastelColorBkgnd(groupColorDefault);
            noteSpecific.setAttribute('data-group-id', 'null');
            return;
        }

        const group = document.getElementById(groupID);
        if (group) {
            const groupColorElement = group.querySelector(".group-color-selector");
            const groupColor = groupColorElement ? window.getComputedStyle(groupColorElement).backgroundColor : "Color no encontrado";
            const groupName = group.querySelector(".group-name").textContent;
            
            noteSpecific.style.backgroundColor = toPastelColorBkgnd(groupColor);
            
            noteGroupColor.style.backgroundColor = groupColor;
            noteGroupName.textContent = groupName;
            noteSpecific.setAttribute('data-group-id', groupID);

        } else {
            console.error(`Error: No se encontró el grupo con ID '${groupID}' para la nota '${noteID}'.`);
            noteSpecific.style.backgroundColor = toPastelColorBkgnd(groupColorDefault);
            noteGroupColor.style.backgroundColor = groupColorDefault;
            noteSpecific.setAttribute('data-group-id', 'null');
        }
    } else {
        console.log("No se encunetran los contenedores de grupo en la nota");
    }
...
//  NO  ME SIRVE, NI SIQUIERA NECESITO USALA
...
const togglePinEstado = async (noteId) => {
    ...
    nota.pinned = !nota.pinned; // Invierte el estado
    // nota.updatedAt = new Date().toISOString(); // Opcional: actualizar fecha para ordenamiento

    await guardarNotaEnDB(nota); // Guarda en la DB
    actualizarPinUI(noteId, nota.pinned); // Actualiza el icono del pin
    ...
    verificarYReubicarNota(noteId, nota.pinned);
};
...
export const verificarYReubicarNota = (noteId, isPinned) => {
    // Asumimos que el elemento de la nota tiene un atributo data-id que coincide con el noteId.
    const noteElement = document.getElementById(noteId);
    console.log(noteElement);
    
    if (!noteElement) return;

    // Obtenemos la instancia del item de Muuri desde los grids.
    ...
    if (isPinned && itemInUnpinned) {
        // La nota está fijada pero se encuentra en el grid de no fijadas. Mover a 'pinned'.
        ...
        // La nota no está fijada pero se encuentra en el grid de fijadas. Mover a 'unpinned'.
...
const eliminarNota = async (identificadorNota) => {
    console.log("elimjala", identificadorNota);

    const notaID = document.getElementById(identificadorNota);
    if (!notaID) {
        console.error(`Elemento de nota con ID ${identificadorNota} no encontrado.`);
        return;
    }

    // Obtenemos el item de Muuri a partir del elemento del DOM
    const item = gridUnpinned.getItem(notaID);
    if (!item) {
        console.error("El elemento no es un item de Muuri válido:", notaID);
        return;
    }

    try {
        // Una vez oculto, lo eliminamos de la DB
        await eliminarNotaDeDB(identificadorNota);
        // Y finalmente lo eliminamos de la instancia de Muuri y del DOM
        gridUnpinned.remove([item], { removeElements: true });
        console.log(`Nota ${notaID} eliminada correctamente.`);
    } catch (error) {
        console.error(`Error en el proceso de eliminación de la nota ${notaID}:`, error);
        // Si algo falla, mostramos el item de nuevo
        // grid.show(item);
    }
};
...
export const duplicarNota = async (noteId) => {
    try {
        // 1. Obtener la nota original de la base de datos
        const notaOriginal = await obtenerNotaPorIdDesdeDB(noteId);
        if (!notaOriginal) {
        ...
        // 4. Renderizar la nueva nota en el DOM para que aparezca inmediatamente
        renderizarNotaEnDOM(notaDuplicada);
    ...
    noteOptionsDropdown.addEventListener("click", async (event) => {
        ...
        if (selectedOptionElement) {
            ...
            switch (selectedAction) {
                case "delete-note":                
                    try {
                        // Esperamos a que la nota se elimine completamente (animación y DB)
                        await eliminarNota(targetNotaId);
                    } catch (error) {
                        // El error ya se loguea dentro de la función eliminarNota
...
let startX, startY;
const allNotesContainers = document.querySelectorAll('.grid, #pinned-notes-container');
